...
def main(mode, in_f, out_f, secret_f,time_list):
    start_time = time.time()
    file = cv2.imread(in_f)
    steg = LSBSteg(file)
    if mode == 'encode':
...
if __name__ == "__main__":
    ...
    mse_list = []
    psnr_list = []
    for file in os.listdir(in_f):
       main(mode, f"{in_f}/{file}", f"{out_f}/{file}", secret_f,time_list)
    avg_time = np.mean(time_list)
    print('Average execution time:', avg_time)
    calculate(in_f,out_f,mse_list,psnr_list)
