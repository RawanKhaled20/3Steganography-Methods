 ...
 */
"""
""" /** Modification author: Rawan Khaled  https://github.com/RawanKhaled20/3Steganography-Methods.git """
import cv2
import os
import sys
...
class file_bits_reader:
    ...
    def __init__(self, f_path):
        try:
            ...
            of the data. """
            self.data = PVD_MAGIC + PVD_VERSION + list(
                data_len.to_bytes(PVD_MAX_LENGTH_FIELD, PVD_BYTE_ORDER)) + self.data
                    ...
                    for h_j in range(height_itr, height_itr + 3):
                        for w_i in range(width_itr, width_itr + 3):
                            ...
                            for rgb in range(3):
                                ...
                                if magic_extracted and (
                                        encoded_size + PVD_HEADER_SIZE) == bits_writer.bytes_wrote_to_file_so_far:
                                    eof_reached = True

                                """ If we have completed reading the header? """
                                if (bits_writer.bytes_wrote_to_file_so_far >= (
                                PVD_HEADER_SIZE)) and magic_extracted == False:
                                    ...
                                    if pvd_magic != PVD_MAGIC or pvd_versn != PVD_VERSION:
                                        raise ValueError(
                                            "Invalid version or image... magic: {} versn: {}".format(pvd_magic,
                                                                                                     pvd_versn))
                                    size_arr = magic[-4:]
                                    """ Parse the encoded data size in the image """
                                    encoded_size = (size_arr[0] << 24) + (size_arr[1] << 16) + (size_arr[2] << 8) + (
                                                size_arr[3] << 0)
...
def run(mode, ref_image_path, secret_op_file, pvd_img_path,time_list):
    start_time = time.time()
    pvd_obj= pvd_lib()
    if mode.lower() == 'e':
        pvd_obj.pvd_embed(ref_image_path, secret_op_file, pvd_img_path)
    elif mode.lower() == 'd':
        pvd_obj.pvd_extract(ref_image_path, secret_op_file, pvd_img_path)
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    psnr_list = []
    for file in os.listdir(folder):
        run(mode, f"{folder}/{file}", secret, f"{out}/{file}",time_list)

    avg_time = np.mean(time_list)
    print('Average execution time:', avg_time)
    calculate(folder,out,mse_list,psnr_list)
